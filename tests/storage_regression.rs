use textcompletion::model::Snippet;
...
fn persists_and_finds_snippet_by_trigger() {
    ...
    let mut snippet = Snippet::personal(";agree", "I agree with the determination.");
...
fn search_matches_title_category_trigger_and_replacement() {
    ...
    let mut md = Snippet::personal(";p2p", "Peer-to-peer review completed.");
    md.title = "Peer to peer".into();
    md.category = "Medical Director".into();
    db.upsert(&md).unwrap();

    let mut rn = Snippet::personal(";outreach", "Member outreach attempted.");
...
fn disabled_snippet_does_not_expand() {
    ...
    let mut snippet = Snippet::personal(";off", "Should not expand");
